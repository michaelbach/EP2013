...
// The following is a NSBezierPath category to allow for rounded corners of the border
...
-(void)appendBezierPathWithRoundedRect:(NSRect)rect cornerRadius:(float)radius {
    if(!NSIsEmptyRect(rect)) {
		if(radius > 0.0) {
			...
			float clampedRadius = fmin(radius, 0.5 * fmin(rect.size.width, rect.size.height));
...
-(void) genTexture; { // generates the texture without drawing texture to current context
	NSImage * image;  NSBitmapImageRep * bitmap;
	NSSize previousSize = texSize;
	
	if((!staticFrame) &&(0.0f == frameSize.width) &&(0.0f == frameSize.height)) { // find frame size if we have not already found it
	...
	if([boxColor alphaComponent]) { // this should be == 0.0f but need to make sure
		[boxColor set]; 
		NSBezierPath *path = [NSBezierPath bezierPathWithRoundedRect:NSInsetRect(NSMakeRect(0.0f, 0.0f, frameSize.width, frameSize.height) , 0.5, 0.5) cornerRadius:cRadius];
		[path fill];
	}
	...
	[image unlockFocus];
	texSize.width = [bitmap pixelsWide];  texSize.height = [bitmap pixelsHigh];
	...
	[bitmap release]; [image release];  doesRequireUpdate = NO;
}
...
-(void) setTextColor:(NSColor *)color { // set default text color
	[color retain];  [textColor release];  textColor = color;  doesRequireUpdate = YES;
}
...
-(void) setBoxColor:(NSColor *)color { // set default text color
	[color retain];  [boxColor release];  boxColor = color;  doesRequireUpdate = YES;
}
...
-(void) setBorderColor:(NSColor *)color { // set default text color
	[color retain];  [borderColor release];  borderColor = color;  doesRequireUpdate = YES;
}
...
-(void) setMargins:(NSSize)size { // set offset size and size to fit with offset
	marginSize = size;
	if (!staticFrame) { // ensure dynamic frame sizes will be recalculated
		frameSize.width = 0.0f;  frameSize.height = 0.0f;
	}
	doesRequireUpdate = YES;
}
...
-(void) useStaticFrame:(NSSize)size { // set static frame size and size to frame
	frameSize = size;  staticFrame = YES;  doesRequireUpdate = YES;
}

-(void) useDynamicFrame {
	if(staticFrame) { // set to dynamic frame and set to regen texture
		staticFrame = NO;
		frameSize.width = 0.0f; frameSize.height = 0.0f; // ensure frame sizes will be recalculated
		doesRequireUpdate = YES;
	}
}
...
-(void) setString:(NSAttributedString *)attributedString { // set string after initial creation
	[attributedString retain];  [string release];  string = attributedString;
...
-(void) drawWithBounds:(NSRect)bounds {
	if (doesRequireUpdate)  [self genTexture];
	if (texName) {
...
-(void) drawAtPoint:(NSPoint)point {
	if (doesRequireUpdate)  [self genTexture]; // ensure size is calculated for bounds
